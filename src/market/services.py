...
def get_daily_stock_quotes_queryset(ticker, days=28, use_bucket=False):
    ...
    lastest_daily_timestamps = (
        StockQuote.objects.filter(
            ...
            time__range=(start_date - timedelta(days=40), end_date),
        )
...
def get_daily_moving_averages(ticker, days=28, queryset=None):
    ...
    if ma_5 <= 0 or ma_20 <= 0:
        return None
    return {"ma_5": float(round(ma_5, 4)), "ma_20": float(round(ma_20, 4))}
...
def calculate_rsi(ticker, days=28, queryset=None, period=14):
    ...
    movement = queryset.annotate(
        closing_price=ExpressionWrapper(
            F("close_price"), output_field=DecimalField(max_digits=10, decimal_places=4)
        ),
        prev_close=Window(
        ...
        gain=Case(
            ...
            default=Value(
                0, output_field=DecimalField(max_digits=10, decimal_places=4)
            ),
            output_field=DecimalField(max_digits=10, decimal_places=4),
        ),
        loss=Case(
    ...
    initial_avg = movement.exclude(prev_close__isnull=True)[:period].aggregate(
        avg_gain=Coalesce(
            ExpressionWrapper(
                Avg("gain"), output_field=DecimalField(max_digits=10, decimal_places=4)
            ),
            Value(0, output_field=DecimalField(max_digits=10, decimal_places=4)),
        ),
        avg_loss=Coalesce(
            ExpressionWrapper(
                Avg("loss"), output_field=DecimalField(max_digits=10, decimal_places=4)
            ),
            Value(0, output_field=DecimalField(max_digits=10, decimal_places=4)),
        ),
    )
    ...
    subsequent_data = list(
        movement.exclude(prev_close__isnull=True)[period:].values("gain", "loss")
    )
